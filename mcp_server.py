...
def setup_mcp_server(kali_client: KaliToolsClient) -> FastMCP:
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # CVE RAG TOOLS — local 320k CVE database retrieval for report enrichment
    # ─────────────────────────────────────────────────────────────────────────

    try:
        from cve_rag import CVESearchEngine
        _cve_engine = CVESearchEngine()
    except Exception as _e:
        _cve_engine = None
        logger.warning(f"CVE RAG engine not available: {_e}. Run: python cve_rag.py --build")

    @mcp.tool(name="cve_search_product")
    def cve_search_product(product: str, version: str = "", limit: int = 10) -> str:
        """
        Search the local CVE database for vulnerabilities affecting a specific product/version.
        Use this after nmap/nikto/whatweb discovers a service to enrich the pentest report.

        Args:
            product: Product or software name (e.g. "apache", "openssh", "wordpress", "mysql")
            version: Version string (e.g. "2.4.49", "7.4", "5.8") — optional but improves accuracy
            limit: Max results to return (default: 10)

        Returns:
            Formatted CVE list with severity, CVSS scores, CWE, and references

        Examples:
            cve_search_product("apache", "2.4.49")
            cve_search_product("openssh", "7.4")
            cve_search_product("log4j")
        """
        if _cve_engine is None:
            return "❌ CVE index not built. Run: python cve_rag.py --build"
        if not _cve_engine.is_ready():
            return "❌ CVE index not found. Run: python cve_rag.py --build"

        results = _cve_engine.search_by_product(product, version, limit=limit)
        if not results:
            return f"No CVEs found for '{product} {version}'.strip()"

        report = f"\n{'='*70}\nCVE INTELLIGENCE: {product.upper()} {version}\n{'='*70}\n"
        report += f"Found {len(results)} CVE(s) — sorted by CVSS score\n\n"
        report += _cve_engine.format_for_report(results)
        report += f"\n{'='*70}\n"
        return report

    @mcp.tool(name="cve_lookup_id")
    def cve_lookup_id(cve_id: str) -> str:
        """
        Look up a specific CVE by its ID for detailed information.
        Use this when a scan tool (nuclei, searchsploit) reports a specific CVE ID.

        Args:
            cve_id: The CVE identifier (e.g. "CVE-2021-44228", "CVE-2023-44487")

        Returns:
            Full CVE details: description, CVSS, CWE, affected versions, references
        """
        if _cve_engine is None:
            return "❌ CVE index not built. Run: python cve_rag.py --build"

        result = _cve_engine.get_by_id(cve_id)
        if not result:
            return f"CVE '{cve_id}' not found in local database."

        score = f"{result['cvss_score']}" if result.get("cvss_score") else "N/A"
        severity = result.get("severity", "UNKNOWN")
        products = ", ".join(result.get("products", [])) or "N/A"
        vendors = ", ".join(result.get("vendors", [])) or "N/A"
        cwes = ", ".join(result.get("cwes", [])) or "N/A"
        versions = ", ".join(result.get("versions", [])[:5]) or "N/A"
        refs = result.get("references", [])

        report = f"\n{'='*70}\n{result['cve_id']} — {severity}\n{'='*70}\n"
        report += f"CVSS Score  : {score}\n"
        report += f"CVSS Vector : {result.get('cvss_vector', 'N/A')}\n"
        report += f"CWE         : {cwes}\n"
        report += f"Vendor      : {vendors}\n"
        report += f"Product     : {products}\n"
        report += f"Versions    : {versions}\n"
        report += f"Published   : {result.get('date_published', 'N/A')}\n"
    ...
    @mcp.tool(name="cve_search_keyword")
    def cve_search_keyword(query: str, limit: int = 10) -> str:
        """
        Free-text search across all CVE descriptions and product names.
        Use for queries like 'sql injection mysql', 'rce apache', 'privilege escalation linux'.

        Args:
            query: Search terms (e.g. "remote code execution nginx", "xss wordpress plugin")
            limit: Max results (default: 10)

        Returns:
            Matching CVEs sorted by CVSS score
        """
        if _cve_engine is None:
            return "❌ CVE index not built. Run: python cve_rag.py --build"

        results = _cve_engine.search_by_keyword(query, limit=limit)
        if not results:
            return f"No CVEs found for query: '{query}'"

        report = f"\n{'='*70}\nCVE SEARCH: {query}\n{'='*70}\n"
        report += f"Found {len(results)} result(s)\n\n"
        report += _cve_engine.format_for_report(results)
        return report

    @mcp.tool(name="cve_enrich_scan_results")
    def cve_enrich_scan_results(services: str, limit_per: int = 5) -> str:
        """
        Batch-enrich multiple discovered services with CVE intelligence in one call.
        Feed this the nmap/nikto output services list to get CVEs for all at once.

        Args:
            services: Comma-separated list of "product version" pairs
                      e.g. "apache 2.4.49, openssh 7.4, mysql 5.7.32, php 7.4.3"
            limit_per: CVEs per service (default: 5)

        Returns:
            CVE intelligence for all services — ready to paste into pentest report
        """
        if _cve_engine is None:
            return "❌ CVE index not built. Run: python cve_rag.py --build"
        ...
        results = _cve_engine.search_multiple_products(service_list, limit_per=limit_per)

        report = f"\n{'='*70}\nCVE INTELLIGENCE REPORT — {len(service_list)} service(s)\n{'='*70}\n"
        total_found = 0

        for service, cves in results.items():
            report += f"\n[{service.upper()}] — {len(cves)} CVE(s) found\n"
            report += "─" * 60 + "\n"
            if cves:
                report += _cve_engine.format_for_report(cves)
                total_found += len(cves)
            else:
                report += "  No CVEs found in local database.\n"

        report += f"\n{'='*70}\n"
        report += f"Total CVEs retrieved: {total_found} across {len(service_list)} services\n"
        report += f"{'='*70}\n"
        return report

    @mcp.tool(name="cve_get_by_severity")
    def cve_get_by_severity(severity: str, year: str = "", limit: int = 20) -> str:
        """
        Get top CVEs by severity level, optionally filtered by year.
        Useful for historical context sections in pentest reports.
        ...
        Returns:
            Top CVEs for the given severity, sorted by CVSS score
        """
        if _cve_engine is None:
            return "❌ CVE index not built. Run: python cve_rag.py --build"

        results = _cve_engine.get_by_severity(severity, year=year, limit=limit)
        if not results:
            return f"No {severity} CVEs found{' for ' + year if year else ''}."
    ...
    def cve_database_stats() -> str:
        """
        Show CVE database statistics — total CVEs, breakdown by severity and year.
        Use this for the 'CVE Database Coverage' section of the pentest report.

        Returns:
            Database stats: total count, severity breakdown, year distribution, DB size
        """
        if _cve_engine is None:
            return "❌ CVE index not built. Run: python cve_rag.py --build"
        if not _cve_engine.is_ready():
        ...
        stats = _cve_engine.get_stats()
        if "error" in stats:
            return f"❌ Error: {stats['error']}"

        report = f"\n{'='*70}\nCVE DATABASE STATISTICS\n{'='*70}\n"
        report += f"Total CVEs indexed : {stats.get('total_cves', 0):,}\n"
        report += f"Database size      : {stats.get('db_size_mb', 0)} MB\n"
        report += f"Index built at     : {stats.get('built_at', 'unknown')}\n"
        report += f"\nBy Severity:\n"
        for sev, count in stats.get("by_severity", {}).items():
            report += f"  {sev:10s}: {count:,}\n"
        report += f"\nRecent Years:\n"
        for year, count in stats.get("recent_years", {}).items():
            report += f"  {year}: {count:,} CVEs\n"
        report += f"{'='*70}\n"
        return report
